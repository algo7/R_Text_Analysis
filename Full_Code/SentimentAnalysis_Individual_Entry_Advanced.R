...
df$sentiment <- lapply(df$processed_text, get_nrc_sentiment)
...
graph_top_words <- function(emotion, timespan, data_source) {
  ...
  if (dim(df_emo)[1] != 0) {
    # Extract the text column
    docs_emo <- iconv(df_emo$processed_text)
    ...
    plot <- ggplot(top_words_df, aes(x = reorder(word, -freq), y = freq)) +
      ...
      labs(title = paste(
        hotel_name, "Top", percentage * 100,
        "% Most Frequent Words for", emotion, "Comments",
        paste("[", data_source, ":", timespan, "]", sep = "")
      ), x = "Word", y = "Frequency")
...
for (i in 1:length(target_year)) {
  graph_top_words("Positive", target_year[i], "Yelp")
  graph_top_words("Negative", target_year[i], "Yelp")
}
